...
test "main - e.g. input" {
    ...
    try main(in, &out);
    try std.testing.expectEqualSlices(u8, "MCD\n", out_buffer.items);
}
...
fn rearrangeStacks(comptime IdType: type, crate_stacks: *CrateStacks(IdType), line_iter: anytype) !void {
    while (try line_iter.next()) |line| {
        ...
        try crate_stacks.moveCrates(instruction.count, instruction.from_stack, instruction.to_stack);
    }
}
...
test "CrateStacks.moveCrate" {
    ...
    try crate_stacks.moveCrates(2, 0, 1);
    const expt_stacks = .{ [_]u8{}, [_]u8{ '3', '1', '2' }, [_]u8{}, [_]u8{ '4', '5' } };
...
fn CrateStacks(comptime IdType: type) type {
    return struct {
        ...
        pub fn moveCrates(self: Self, count: usize, from: usize, to: usize) !void {
            ...
            std.mem.reverse(IdType, to_stack.items[to_stack.items.len - count .. to_stack.items.len]);
        }
