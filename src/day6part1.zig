...
test "find start of packet marker - full input" {
    ...
    try main(in, &out);
    try std.testing.expectEqualSlices(u8, "1235\n", out_buffer.items);
}
...
pub fn main(stdin: anytype, stdout: anytype) !void {
    ...
    if (maybe_line) |line| {
        const result = findStartOfPacketMarker(line);
...
test "find start of packet marker - e.g. inputs" {
    const test_cases = .{
        .{ "inputs/day6-input-eg0.txt", 7 },
        .{ "inputs/day6-input-eg1.txt", 5 },
        .{ "inputs/day6-input-eg2.txt", 6 },
        .{ "inputs/day6-input-eg3.txt", 10 },
        .{ "inputs/day6-input-eg4.txt", 11 },
    };
    ...
    inline for (test_cases) |test_case| {
        ...
        const calc_result = findStartOfPacketMarker(line).?;

        try std.testing.expectEqual(@intCast(usize, expt_result), calc_result);
    }
}

fn findStartOfPacketMarker(signal: []const u8) ?usize {
    const WINDOW_LEN = 4;
